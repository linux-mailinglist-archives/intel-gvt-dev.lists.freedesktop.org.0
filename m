...
Received: from gabe.freedesktop.org (gabe.freedesktop.org [131.252.210.177])
	by mail.lfdr.de (Postfix) with ESMTPS id B6149327674
	for <lists+intel-gvt-dev@lfdr.de>; Mon,  1 Mar 2021 04:44:41 +0100 (CET)
Received: from gabe.freedesktop.org (localhost [127.0.0.1])
	by gabe.freedesktop.org (Postfix) with ESMTP id 633076E063;
	Mon,  1 Mar 2021 03:44:40 +0000 (UTC)
X-Original-To: intel-gvt-dev@lists.freedesktop.org
Delivered-To: intel-gvt-dev@lists.freedesktop.org
Received: from mga12.intel.com (mga12.intel.com [192.55.52.136])
 by gabe.freedesktop.org (Postfix) with ESMTPS id 743476E063
 for <intel-gvt-dev@lists.freedesktop.org>;
 Mon,  1 Mar 2021 03:44:39 +0000 (UTC)
IronPort-SDR: 9VuG71RNUnGcbXtMc/Ty8IUNk5iugHm5DI2TzguKck+6W7MyDXBj+F+jH8+0A4SYWtcgwBOwmI
 LHfUpnI8uEOw==
X-IronPort-AV: E=McAfee;i="6000,8403,9909"; a="165577760"
X-IronPort-AV: E=Sophos;i="5.81,214,1610438400"; 
 d="asc'?scan'208";a="165577760"
Received: from fmsmga004.fm.intel.com ([10.253.24.48])
 by fmsmga106.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384;
 28 Feb 2021 19:44:38 -0800
IronPort-SDR: g2TfoqoCCS62liGSfsyJUws4PnnLF05f5WeHdeGsThOSO18hMVk3bRv7raf2b29e4VW0oFSpuA
 5XOUbfvh+xbA==
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.81,214,1610438400"; 
 d="asc'?scan'208";a="426788262"
Received: from zhen-hp.sh.intel.com (HELO zhen-hp) ([10.239.160.147])
 by fmsmga004.fm.intel.com with ESMTP; 28 Feb 2021 19:44:37 -0800
Date: Mon, 1 Mar 2021 11:28:27 +0800
From: Zhenyu Wang <zhenyuw@linux.intel.com>
To: Colin Xu <colin.xu@intel.com>
Subject: Re: [PATCH v2 2/2] drm/i915/gvt: Refactor GVT vblank emulator for
 vGPU virtual display
Message-ID: <20210301032827.GQ1551@zhen-hp.sh.intel.com>
References: <20210226044630.284269-1-colin.xu@intel.com>
MIME-Version: 1.0
In-Reply-To: <20210226044630.284269-1-colin.xu@intel.com>
...
Content-Type: multipart/mixed; boundary="===============0635987058=="
...
> @@ -516,11 +516,27 @@ static void clean_virtual_dp_monitor(struct intel_v=
...
>  		return -EINVAL;
> @@ -547,47 +563,56 @@ static int setup_virtual_dp_monitor(struct intel_vg=
...
> +	vblank_timer->period =3D DIV64_U64_ROUND_CLOSEST(NSEC_PER_SEC * MSEC_PE=
R_SEC, vblank_timer->vrefresh_k);
> +
...
> +			vblank_timer->vrefresh_k =3D port->vrefresh_k;
> +			vblank_timer->period =3D DIV64_U64_ROUND_CLOSEST(NSEC_PER_SEC * MSEC_=
PER_SEC, vblank_timer->vrefresh_k);
...
> @@ -619,7 +644,7 @@ static void emulate_vblank_on_pipe(struct intel_vgpu =
*vgpu, int pipe)
>  	}
>  }
> =20
...
>  {
>  	switch (id) {
> @@ -205,8 +212,8 @@ static inline unsigned int vgpu_edid_yres(enum intel_=
vgpu_edid id)
>  	}
>  }
> =20
...
>  				(void *)&gvt->service_request) ||
> @@ -278,7 +292,6 @@ void intel_gvt_clean_device(struct drm_i915_private *=
i915)
...
> @@ -337,7 +350,7 @@ int intel_gvt_init_device(struct drm_i915_private *i9=
15)
> =20
>  	ret =3D intel_gvt_init_gtt(gvt);
>  	if (ret)
> -		goto out_clean_irq;
> +		goto out_free_firmware;
> =20
>  	ret =3D intel_gvt_init_workload_scheduler(gvt);
>  	if (ret)
> @@ -392,8 +405,6 @@ int intel_gvt_init_device(struct drm_i915_private *i9=
15)
>  	intel_gvt_clean_workload_scheduler(gvt);
>  out_clean_gtt:
>  	intel_gvt_clean_gtt(gvt);
...
>  	u32 scan_nonprivbb;
>  };
> @@ -347,13 +348,16 @@ static inline struct intel_gvt *to_gvt(struct drm_i=
915_private *i915)
>  }
> =20
>  enum {
...
> @@ -703,14 +703,11 @@ static int pipeconf_mmio_write(struct intel_vgpu *v=
...
> - * @gvt: a GVT device
> - *
> - * This function is called at driver unloading stage, to clean up GVT-g =
IRQ
> - * emulation subsystem.
> - *
> - */
...
iF0EARECAB0WIQTXuabgHDW6LPt9CICxBBozTXgYJwUCYDxfVgAKCRCxBBozTXgY
J68MAKCCRf+1mOZVjyZhISg3jl16yVuUOACfQPHsaDYJ+6DSoOCt8Z+wruoaA3U=
=mPT8
-----END PGP SIGNATURE-----
